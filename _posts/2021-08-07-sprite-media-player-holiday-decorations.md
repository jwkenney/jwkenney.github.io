...
* If playing in trigger mode with a motion sensor, you can pick either "Trigger Low" or "Trigger High", to activate your trigger video when your sensor activates or resets. *Update:* Team Kingsley weighed in on this, and noted that the motion sensor's steady state is "Low," and it briefly switches to "High" when it detects motion- then it transitions back to "Low". They recommend setting this to "Trigger High" for the motion sensor, for the best reaction time. For push-button or pressure-switch sensors, "Low" usually means a button *press*, and "High" usually means a button *release*. Experiment to see what gives you the best results.
